...
int main( int argc, char** argv )
{
  ...
  Mat img, grayImg;			// image objects
  VideoCapture cap; 		// capture object

  const string windowName = "Histogram Based Recognition"; // window name
  ...
  int channels[] = {0};   // we compute the histogram from the 0-th channel only

  // create the histogram explicitlty, specifying 32-bit float (CV_32F) data
  // storage

  MatND currentHistogram;
  currentHistogram.create(1, hist_size, CV_32F);
		  ...
		  if (key == 'x'){
			...
			if (!recognitionStage) {
				 if (imagesCollected < MAX_NUMBER_OF_SAMPLE_IMAGES)
				 {
				    ...
				    input[imagesCollected] = grayImg.clone();

					calcHist(&grayImg, 1, channels, Mat(), currentHistogram, 1,
